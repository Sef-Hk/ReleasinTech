import type { LetstalkInter as LetsTalkPops } from "@/payload-types"
import Link from "next/link"

export const LetsTalk: React.FC<LetsTalkPops> = ({ link }) => {
  const label = link.label || ''
  const repeatedLabel = Array(4).fill(label).join(' <span class="mx-4 text-gray-400 text-[24px]">♦</span> ')

  return (
    <Link href={link.url || '#'} className="block no-underline">
